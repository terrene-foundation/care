...
function resolveLearningDir(cwd) {
  ...
  return path.join(os.homedir(), ".claude", "co-learning");
}

/**
 * Detect project system type for identity tagging.
 *
 * @param {string} [cwd] - Project working directory
 * @returns {string} System type identifier
 */
function detectSystemType(cwd) {
  if (!cwd) return "co";
  try {
...
/**
 * Get focus areas appropriate for the system type.
 *
 * @param {string} systemType
 * @returns {string[]}
 */
function getFocusAreas(systemType) {
  if (systemType === "orchestrator") {
    return [
      "cross-repo-patterns",
      "content-flow",
      "sync-status",
...
function ensureLearningDir(cwd) {
  ...
  if (!fs.existsSync(identityFile)) {
    try {
      const systemType = detectSystemType(cwd);
      const identity = {
        system: systemType,
        version: "2.0.0",
        ...
        per_project: true,
        focus_areas: getFocusAreas(systemType),
      };
