...
Commands:
    ...
    care-platform org export --org-id <id> --output <f>  Export org to YAML
    care-platform org import-file --file <f>             Import org from YAML/JSON
    care-platform org diff <file1> <file2>               Compare two org definitions
    care-platform org deploy --file <f>                  Deploy org to trust store
    care-platform org status                             Show runtime org health
"""
...
def org_export(org_id: str, output: str) -> None:
    ...
    console.print(
        Panel(
            ...
            f"  Envelopes: {len(org_def.envelopes)}\n"
            f"  Workspaces: {len(org_def.workspaces)}",
            title="CARE Platform - Org Exported",
            border_style="green",
        )
    )
...
def org_import_file(file_path: str) -> None:
    ...
    from care_platform.org.builder import OrgDefinition
    ...
    if valid:
        console.print(
            Panel(
                ...
                title="CARE Platform - Org Import",
                border_style="green",
            )
        )
    else:
        console.print(
            Panel(
                ...
                + "\n".join(f"  - {e}" for e in errors),
                title="CARE Platform - Org Import (Validation Errors)",
                border_style="yellow",
            )
        )
...
def org_deploy(file_path: str) -> None:
    ...
    # Convert OrgDefinition to PlatformConfig for bootstrapping
    ...
    if result.is_successful:
        ...
        console.print(
            Panel(
                ...
                title="CARE Platform - Org Deployed",
                border_style="green",
            )
        )
    else:
        error_console.print(
            Panel(
                ...
                title="CARE Platform - Deploy Errors",
                border_style="red",
            )
        )
        sys.exit(1)
...
def org_status() -> None:
    ...
    console.print(
        Panel(
            ...
            "  [cyan]care-platform org deploy --file org.yaml[/cyan]\n\n"
            "Or create one from a template:\n"
            "  [cyan]care-platform org create --template minimal --name 'My Org'[/cyan]",
            title="CARE Platform - Organization Status",
            border_style="blue",
        )
    )
