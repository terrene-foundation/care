...
"""CARE Platform API endpoint definitions and handler logic.
...
class PlatformAPI:
    ...
    def __init__(
        ...
        *,
        ...
        if registry is None:
            raise ValueError(
                "PlatformAPI requires a non-None AgentRegistry. "
                "Provide a registry instance — do not pass None."
            )
        if approval_queue is None:
            raise ValueError(
                "PlatformAPI requires a non-None ApprovalQueue. "
                "Provide an approval_queue instance — do not pass None."
            )
        if cost_tracker is None:
            raise ValueError(
                "PlatformAPI requires a non-None CostTracker. "
    ...
    def get_envelope(self, envelope_id: str) -> ApiResponse:
        ...
        if self._envelope_registry is None:
            logger.warning("get_envelope: envelope_registry not configured on PlatformAPI")
            return ApiResponse(
                status="error",
                error=(
                    "Dashboard endpoint unavailable: envelope_registry not "
                    "provided to PlatformAPI. Pass envelope_registry to enable "
                    "this endpoint."
                ),
            )
    ...
    def list_workspaces(self) -> ApiResponse:
        ...
        if self._workspace_registry is None:
            logger.warning("list_workspaces: workspace_registry not configured on PlatformAPI")
            return ApiResponse(
                status="error",
                error=(
                    "Dashboard endpoint unavailable: workspace_registry not "
                    "provided to PlatformAPI. Pass workspace_registry to enable "
                    "this endpoint."
                ),
            )
    ...
    def list_bridges(self) -> ApiResponse:
        ...
        if self._bridge_manager is None:
            logger.warning("list_bridges: bridge_manager not configured on PlatformAPI")
            return ApiResponse(
                status="error",
                error=(
                    "Dashboard endpoint unavailable: bridge_manager not "
                    "provided to PlatformAPI. Pass bridge_manager to enable "
                    "this endpoint."
                ),
            )
    ...
    def verification_stats_report(self) -> ApiResponse:
        ...
        if self._verification_stats is None:
            logger.warning(
                "verification_stats_report: verification_stats not configured on PlatformAPI"
            )
            return ApiResponse(
                status="error",
                error=(
                    "Dashboard endpoint unavailable: verification_stats not "
                    "provided to PlatformAPI. Pass verification_stats to enable "
                    "this endpoint."
                ),
            )
    ...
    def create_bridge(self, data: dict[str, Any]) -> ApiResponse:
        ...
        """
        if self._bridge_manager is None:
            logger.warning("create_bridge: bridge_manager not configured on PlatformAPI")
            return ApiResponse(
                status="error",
                error=(
    ...
    def get_bridge(self, bridge_id: str) -> ApiResponse:
        ...
        """
        if self._bridge_manager is None:
            logger.warning("get_bridge: bridge_manager not configured on PlatformAPI")
            return ApiResponse(
                status="error",
                error=(
                    "Bridge management unavailable: bridge_manager not provided to PlatformAPI."
