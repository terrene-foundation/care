...
"""PACT enforcement pipeline — composes GradientEngine + EATP StrictEnforcer.
...
from pact.build.config.schema import VerificationLevel as CareVerificationLevel
from pact.trust.constraint.envelope import EnvelopeEvaluation
...
# | level (enum)         | level (enum)           | See _PACT_TO_EATP_LEVEL          |
...
_PACT_TO_EATP_LEVEL: dict[CareVerificationLevel, EATPVerificationLevel] = {
...
_VERDICT_TO_PACT_LEVEL: dict[Verdict, CareVerificationLevel] = {
...
#   2 violations → HELD
#   valid=False  → BLOCKED
_PACT_LEVEL_VIOLATION_COUNT: dict[CareVerificationLevel, int] = {
...
def care_result_to_eatp_result(
    ...
    ``PACT_FLAG_THRESHOLD``). The ``CareEnforcementPipeline`` sets this
    automatically.
    ...
    violation_count = _PACT_LEVEL_VIOLATION_COUNT[level]
    ...
    return EATPVerificationResult(
        ...
        level=_PACT_TO_EATP_LEVEL[level],
        reason=reason,
...
def verdict_to_care_level(verdict: Verdict) -> CareVerificationLevel:
    ...
    """
    return _VERDICT_TO_PACT_LEVEL[verdict]
    ...
    def __init__(
        ...
        *,
        ...
        kwargs: dict = {
            ...
            "flag_threshold": PACT_FLAG_THRESHOLD,
        }
...
def create_approval_held_callback(
    ...
    Args:
        approval_queue: The PACT approval queue.
