...
"""PACT trust decorators — thin wrappers around EATP SDK enforce operations.

These decorators wire EATP's trust operations (VERIFY, AUDIT, shadow check)
to the PACT's trust infrastructure (EATPBridge, ShadowEnforcer),
...
if TYPE_CHECKING:
    ...
    from pact.trust.shadow_enforcer import ShadowEnforcer as CareShadowEnforcer
...
class CareTrustOpsProvider:
    ...
    Args:
        bridge: The PACT EATPBridge instance. Must be initialized
...
def care_verified(
    ...
    *,
    ...
    """PACT wrapper around EATP VERIFY + StrictEnforcer — pre-execution trust enforcement.
...
def care_audited(
    ...
    *,
    ...
    """PACT wrapper around EATP AUDIT — post-execution audit trail.
...
def care_shadow(
    ...
    *,
    ...
    """PACT wrapper around EATP shadow verification — non-blocking observation.
...
def _forward_to_care_shadow(
    ...
    try:
        ...
        logger.warning(
            "PACT ShadowEnforcer evaluation failed for action=%s agent=%s: %s. "
            "Shadow observation skipped (fail-safe).",
            action,
            agent_id,
            type(exc).__name__,
