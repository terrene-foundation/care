...
"""EATP Bridge — connects PACT models to EATP SDK operations.

This bridge layer translates between PACT configuration models
...
class InMemoryAuthorityRegistry:
    """Simple in-memory authority registry for PACT.
...
class EATPBridge:
    """Bridge between PACT and EATP SDK.

    Translates PACT configuration models into EATP SDK operations,
    ...
    def map_envelope_to_constraints(self, config: ConstraintEnvelopeConfig) -> list[str]:
        ...
        Args:
            config: PACT constraint envelope configuration.
    ...
    async def delegate(
        ...
        """Create a delegation from a PACT agent/authority to another agent.
