...
| Field            | Value                                                                  |
| ---------------- | ---------------------------------------------------------------------- |
| **What**         | Move the entire `pact/` directory to `src/pact/`     |
| **Where**        | `src/pact/` (new location for all 60 Python modules)          |
| **Evidence**     | `ls src/pact/__init__.py` succeeds; `ls pact/` fails |
...
| **Evidence**     | `pip install -e .` succeeds; `python -c "import pact"` works                                                                                                                                                                                                          |
| **Dependencies** | M13-T01                                                                                                                                                                                                                                                                        |
| **Detail**       | Change line 70-71 from `where = ["."]` / `exclude = ["tests*", ...]` to `where = ["src"]` / `exclude = ["tests*", ...]`. The `project.scripts` entry `pact = "pact.cli:main"` is fine -- it references the importable package name, not the filesystem path. |
...
| Field            | Value                                                                                                                                                                                                                                                                            |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **What**         | Run the full test suite; fix any import failures caused by the move. With `pip install -e .` and the `where = ["src"]` config, the package is installed in the venv and imports should resolve as `pact.*` without change. This task is validation, not bulk rewriting. |
...
| **Where**        | New file: `src/pact/trust/lifecycle.py`. Modify: `src/pact/trust/genesis.py`, `src/pact/trust/delegation.py`                                                                                                     |
...
| **Where**        | New file: `src/pact/workspace/bridge_lifecycle.py`. Modify: `src/pact/workspace/bridge.py`                                                                                                                                             |
...
| **Where**        | Modify: `src/pact/workspace/models.py`                                                                                                                                                                                                 |
...
| **Where**        | New file: `src/pact/constraint/resolution.py`                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| **Where**        | New file: `src/pact/trust/uncertainty.py`                                                                                                                                                                                                                      |
...
| **Where**        | New file: `src/pact/resilience/failure_modes.py`                                                                                                                                                                                                                     |
...
| **Where**        | Modify: `src/pact/config/schema.py` (add field to DataAccessConstraintConfig), `src/pact/constraint/envelope.py` (enforce confidentiality in data access evaluation), `src/pact/trust/reasoning.py` (re-export from schema)                                                                                                                                  |
...
| **Where**        | New file: `src/pact/trust/sd_jwt.py`                                                                                                                                                                                                        |
...
| **Where**        | Modify: `src/pact/config/schema.py` (add `reasoning_required: bool` to each dimension config), `src/pact/constraint/envelope.py` (check for reasoning trace when REASONING_REQUIRED), `src/pact/trust/delegation.py` (enforce inheritance in tightening validation) |
...
| **Where**        | Modify: `src/pact/constraint/signing.py`. New utility: `src/pact/trust/jcs.py` (JCS implementation or wrapper around `jcs` PyPI package). Also update `src/pact/audit/anchor.py` hash computation.                                                    |
...
| **Where**        | Modify: `src/pact/trust/reasoning.py` (add `genesis_binding_hash` field, update `compute_hash()` to include genesis reference). New: `src/pact/trust/dual_binding.py` (binding verification logic).                                               |
...
| **Where**        | New file: `src/pact/constraint/enforcer.py`. Modify: `src/pact/execution/runtime.py` (inject enforcer into the execution pipeline).                                                                                                                                                                                                                       |
...
| **Where**        | Modify: `src/pact/trust/attestation.py` (add `has_authorization()` separate from `has_capability()`). New: `src/pact/trust/authorization.py` (AuthorizationCheck combining envelope + attestation). Modify: `src/pact/constraint/middleware.py` (use `AuthorizationCheck` instead of separate checks).                                       |
...
| **Where**        | Modify: `src/pact/persistence/store.py` (add `health_check()` to TrustStore protocol). Modify: `src/pact/persistence/sqlite_store.py` (implement health_check). New: `src/pact/persistence/health.py` (TrustStoreHealthCheck with circuit breaker). Modify: `src/pact/constraint/middleware.py` (check store health before processing).                               |
...
| **Where**        | Modify: `src/pact/org/builder.py` (add `save()` and `load()` methods). Modify: `src/pact/persistence/store.py` (add `store_org_definition()` / `get_org_definition()` to protocol). Implement in `MemoryStore`, `FilesystemStore`, `SQLiteTrustStore`. |
...
| **Where**        | Modify: `src/pact/trust/delegation.py` (add `previous_record_hash` to delegation data). Modify: `src/pact/persistence/sqlite_store.py` (store and verify hashes). New: `src/pact/trust/integrity.py` (TrustChainIntegrity verifier).                                                |
...
| **Where**        | New: `src/pact/workspace/knowledge_policy.py`. Modify: `src/pact/workspace/bridge.py` (integrate policy check into `access_through_bridge()`).                                                    |
...
| **Where**        | Modify: `src/pact/constraint/middleware.py` (add cache lookup before full verification). Modify: `src/pact/constraint/cache.py` (add cache key generation from middleware context).                                                                           |
...
| **Where**        | New: `src/pact/planes/__init__.py`, `src/pact/planes/management.py`, `src/pact/planes/data.py`. Modify: `src/pact/bootstrap.py` (use management plane). Modify: `src/pact/execution/runtime.py` (use data plane).                                                            |
...
| **Where**        | Modify: `src/pact/api/endpoints.py`                                                                                                                                                                                                                                                                                                                                                                        |
...
| **Where**        | New: `src/pact/api/server.py`                                                                                                                                                                   |
| **Evidence**     | `python -m pact.api.server` starts a server; `curl http://localhost:8000/api/v1/teams` returns JSON                                                                                             |
...
| **Where**        | Modify: `src/pact/api/server.py` (add WebSocket route). New: `src/pact/api/events.py` (event bus for real-time notifications). |
...
| **Where**        | `apps/web/lib/api.ts`, `apps/web/types/pact.ts`                                                                                                                |
...
**Goal**: Prove the PACT IS a Constrained Organization by validating all five constitutive properties and three behavioral tests.
...
### M19-T10: End-to-end integration test -- PACT IS a Constrained Organization

| Field            | Value                                                                                                                                                                                                              |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **What**         | Single comprehensive E2E test that proves the PACT is a Constrained Organization: bootstrap a full org, execute agent actions, verify all five properties and three behavioral tests hold simultaneously. |
...
- [ ] The PACT passes all five constitutive property tests
- [ ] The PACT passes all three behavioral tests
