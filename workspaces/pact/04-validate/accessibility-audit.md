# PACT Web Dashboard -- WCAG 2.1 AA Accessibility Audit
...
The PACT dashboard had a solid foundation (semantic HTML in many places, proper `role="dialog"` on modals, good color contrast in most areas) but had systematic gaps in seven categories. All issues have been fixed directly in the source files.
...
a) **Page titles**: Added `title.template: "%s | PACT"` to the root layout metadata so each page can provide a unique title. Client-side pages inherit the default "PACT" title.
