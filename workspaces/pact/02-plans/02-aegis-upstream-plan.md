# PACT — Upstream Enhancements Plan

**Date**: 2026-03-15 (revalidated)
**Brief**: `workspaces/pact/briefs/02-aegis-foundational-strengths.md`
**Analysis**: `workspaces/pact/01-analysis/07-aegis-upstream-revalidation.md`

---

## Key Principles

1. **Capabilities belong where they belong**: Core EATP capabilities belong in kailash-py. PACT adds governance orchestration on top.
...
| --------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| S9 — EATP Decorators                    | **Already in kailash-py** (`packages/eatp/src/eatp/enforce/decorators.py`) | `@verified`, `@audited`, `@shadow` already exist. PACT just needs to consume them. |
...
The PACT plan focuses on **CARE-specific governance extensions** — features that consume the EATP SDK and add organizational governance on top.

---

## Revised Scope: PACT Only

### Tier 1 — Consume EATP SDK Better

#### 1.1 EATP Decorator Integration (S9) — CONSUME, NOT BUILD

**Deliverable**: Wire up existing EATP decorators into PACT trust layer

The decorators already exist in `eatp.enforce.decorators`. The PACT needs to:
...
**Deliverable**: Enhance `pact/constraint/gradient.py`
...
**Deliverable**: Enhance `pact/trust/shadow_enforcer.py`
...
**Deliverable**: Enhance `pact/trust/reasoning.py`
...
**Deliverable**: Enhance `pact/persistence/posture_history.py`

- Reconcile trigger type taxonomy: PACT has 4 types (INCIDENT, REVIEW, SCHEDULED, CASCADE_REVOCATION), Aegis has 9. This is a taxonomy reconciliation, not a simple addition — some types overlap, some are unique to each.
...
- Document fail-closed contract as PACT specification requirement

---

## Removed from PACT Scope
