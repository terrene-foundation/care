...
- CLI commands: `pact org create`, `pact org validate`, `pact org list-templates`
...
1. **CRITICAL: CI paths wrong** — `ci.yml` references `pact/` not `src/pact/` (lines 33-34, 39)
...
The PACT declares all four Kailash frameworks as dependencies (`kailash`, `kailash-nexus`, `kailash-dataflow`, `kailash-kaizen`) in `pyproject.toml`, but currently uses none of them in source code. The entire platform is built as pure Python with Pydantic models, hand-rolled FastAPI, Click CLI, and custom persistence stores.
...
3. **CI path bug**: `ci.yml` lint/mypy targets `pact/` but source is `src/pact/`. Likely silently passing or not running.
