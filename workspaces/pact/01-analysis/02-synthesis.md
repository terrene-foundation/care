...
PACT workspace owns the Foundation's operational model.
...
| **Enterprise prospects** | PACT is a reference implementation they can evaluate | "Does this work for a real organization?" — yes, the Foundation runs on it |
...
| `workspaces/pact/` | Platform Development | Platform design, architecture, implementation |
...
The PACT addresses **single-organization governance** — running one organization with AI agents under EATP trust governance:

| Capability | PACT (Foundation, Apache 2.0) |
|-----------|---------------------------------------|
...
Multi-organization capabilities (cross-org trust federation, distributed audit, federated knowledge) are naturally more complex and beyond the PACT's scope. Since the standards and reference model are open, commercial vendors can build multi-org products on top — the Foundation has no involvement in or relationship with any such products.
...
The highest-confidence finding across all four agents: **the existing COC setup is already a partial PACT implementation**. The risk of "building something new from scratch" is low because the foundation (literally) already exists.

---

## Architecture Gap (5 Gaps to Close)

The deep-analyst identified five gaps between the current COC setup and a full PACT:
...
- `docs/00-anchor/03-ip-ownership.md` — Clarify PACT IP ownership
...
- `.claude/rules/terrene-naming.md` — Add PACT terminology
...
| 2 | Build vs compose | **Compose from Kailash Python** (RESOLVED) | Kailash IS the Foundation's open-source stack. PACT uses Kaizen, DataFlow. |
| 3 | Technical blueprint | **Implementation-neutral** (RESOLVED) | Blueprint must be stack-agnostic. PACT is the reference implementation. |
...
- **C-2**: CARE technical blueprint (`docs/02-standards/care/05-implementation/03-technical-blueprint.md`) still describes a specific commercial product ("Agentic OS"). Must be made implementation-neutral before PACT launches.
- **H-1**: The "nascent PACT" claim should be qualified — the COC setup demonstrates the organizational structure but lacks trust enforcement, persistence, multi-user capability, and runtime independence. The five architecture gaps are the majority of actual engineering work.
- **H-2**: Competitive window assessment needed — well-funded competitors (LangChain, Google, Microsoft) may add "good enough" governance before PACT matures.
- **H-3**: ShadowEnforcer is referenced but never defined in PACT context — it's an EATP concept requiring SDK Phase 2+.
...
To prevent scope creep, the PACT is explicitly NOT:
