...
**Deep-dived**: kailash-py EATP SDK, PACT consumption patterns

---

## Initiative Summary

Evaluate 12 foundational strengths for upstream to the PACT. After deep-diving the kailash-py EATP SDK, the initiative split into two tracks:

1. **Kailash Python SDK gaps** — 11 core EATP capabilities that belong at the SDK layer
2. **PACT enhancements** — 7 CARE-specific governance improvements that consume the EATP SDK

**Note**: See `07-aegis-upstream-revalidation.md` for the final revalidated analysis with 3 material corrections.

---

## Key Discovery: The PACT Already Consumes EATP Correctly

The audit confirmed that the PACT is NOT duplicating the EATP SDK:
...
**Notable**: S9 (EATP decorators) — `@verified`, `@audited`, `@shadow` — already exist in `packages/eatp/src/eatp/enforce/decorators.py`. The PACT analysis originally planned to build these from scratch. Not needed.

### PACT — 7 Enhancements (Revalidated)
...
| ---------------------------------------- | ----------------------------------------- | ----------------------------------------------- |
| Build EATP decorators in PACT   | Consume existing decorators from EATP SDK | They already exist in kailash-py                |
...
| 12 items in PACT                | 7 items in CARE + 11 gaps in kailash-py   | Capabilities go where they belong               |
