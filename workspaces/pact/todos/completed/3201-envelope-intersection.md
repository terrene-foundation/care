...
Implement `compute_bridge_envelope(source_envelope, bridge_constraints, target_envelope) -> ConstraintEnvelopeConfig` in a new `src/pact/constraint/bridge_envelope.py` module. This function takes three `ConstraintEnvelopeConfig` objects and returns the most restrictive combination across all five CARE constraint dimensions.
...
- `src/pact/constraint/bridge_envelope.py` (new file — primary implementation)

## Evidence

- [ ] `src/pact/constraint/bridge_envelope.py` exists with `compute_bridge_envelope()` function
