...
`AuditPipeline` implemented in `pact/audit/pipeline.py` connecting action execution to tamper-evident audit chain.

- `pact/audit/pipeline.py` — `AuditPipeline` with `record_action()`, `verify_chain_integrity()`, `export_chain()`
