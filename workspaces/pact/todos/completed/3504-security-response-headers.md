...
Create `src/pact/api/security_headers.py` with a `SecurityHeadersMiddleware` class (extending `starlette.middleware.base.BaseHTTPMiddleware`) that appends the following headers to every response:
...
Register `SecurityHeadersMiddleware` in `src/pact/api/server.py` using `app.add_middleware()` so it wraps all routes including health endpoints.

## Where

- `src/pact/api/security_headers.py` (new)
- `src/pact/api/server.py` (register middleware)

## Evidence

- [ ] `src/pact/api/security_headers.py` exists with `SecurityHeadersMiddleware`
