...
Add bridge management API endpoints to the PACT FastAPI application. All endpoints require authentication.
...
Wire all endpoints into the existing `PlatformAPI` class in `src/pact/api/endpoints.py` by adding corresponding methods, and register the routes in `src/pact/api/server.py`.
...
- `src/pact/api/endpoints.py` (add `PlatformAPI` methods)
- `src/pact/api/server.py` (register routes)
