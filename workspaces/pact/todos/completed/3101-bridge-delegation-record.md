...
The dataclass fields are: `bridge_id`, `source_team_id`, `target_team_id`, `bridge_type` (using the existing `BridgeType` enum from `pact/workspace/bridge.py`), `delegation_record` (the underlying `eatp.chain.DelegationRecord`), `created_at`, and `expires_at`.
...
Extend the `TrustStore` protocol (in `pact/trust/eatp_bridge.py` or wherever the protocol lives) with a `store_bridge_delegation(delegation: BridgeDelegation) -> None` method and a `get_bridge_delegations(bridge_id: str) -> list[BridgeDelegation]` method. Implement both on `SQLiteTrustStore`.

## Where

- `src/pact/trust/bridge_trust.py` (new file — primary implementation)
- `src/pact/trust/eatp_bridge.py` (extend TrustStore protocol if needed)

## Evidence

- [ ] `src/pact/trust/bridge_trust.py` exists with `BridgeDelegation` dataclass defined
