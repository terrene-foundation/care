...
Generate and commit `examples/` directory with reference YAML org definitions that users can use with `pact org import`. Include: foundation org (full multi-team), minimal org (single team), and one custom template example.
...
- `pact org import --file examples/foundation-org.yaml` succeeds
- `pact org validate --file examples/minimal-org.yaml` passes
