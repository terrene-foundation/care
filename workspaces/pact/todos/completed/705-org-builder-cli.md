...
- [ ] Implement `pact team add` command:
...
- [ ] Implement `pact team establish` command:
...
- [ ] Implement `pact validate` command:
...
- `pact init` creates genesis record end-to-end
- `pact team add` + `establish` produces functioning team
- `pact status` shows accurate platform state
- `pact validate` catches all schema errors
...
- Existing CLI: `pact/cli/__init__.py` (from 112)
