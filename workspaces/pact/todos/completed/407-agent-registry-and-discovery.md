...
**Verified by**: AgentRegistry + AgentRecord + AgentStatus + find_by_capability + staleness detection in `pact/execution/registry.py`; 25 unit tests pass in `tests/unit/execution/test_registry.py`
...
- [ ] Implement `pact/execution/registry.py`:
...
- [ ] Implement `pact agents list [--team dm] [--status active]` CLI command
