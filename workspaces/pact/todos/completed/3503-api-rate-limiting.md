...
Add rate limiting middleware to the FastAPI application using `slowapi` to prevent abuse and denial-of-service against the PACT API.

Add `slowapi` to `pyproject.toml` dependencies.

Create `src/pact/api/rate_limit.py` with:
...
Integrate into `src/pact/api/server.py`:
...
Add `CARE_RATE_LIMIT_GET` and `CARE_RATE_LIMIT_POST` to `EnvConfig` in `src/pact/config/env.py` with documented defaults.

## Where

- `src/pact/api/rate_limit.py` (new)
- `src/pact/api/server.py` (integrate limiter and exception handler)
- `src/pact/config/env.py` (add `CARE_RATE_LIMIT_GET`, `CARE_RATE_LIMIT_POST`)
- `pyproject.toml` (add `slowapi` dependency)

## Evidence

- [ ] `slowapi` present in `pyproject.toml` dependencies
- [ ] `src/pact/api/rate_limit.py` exists with `Limiter` and `RateLimitExceededHandler`
