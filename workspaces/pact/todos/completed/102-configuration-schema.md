...
Define the configuration schema for the PACT — how organizations describe their structure, teams, agents, constraint envelopes, and workspace layout. This is the entry point for platform configuration.

## Tasks

- [ ] Design configuration file format (YAML recommended — readable, editable, version-controllable)
- [ ] Define `pact/config/schema.py` — Pydantic models for:
...
- [ ] Create `pact/config/loader.py` — Load and validate YAML config files
- [ ] Create `pact/config/defaults.py` — Sensible defaults for optional fields
...
- `pact/config/__init__.py` — Module entry point
