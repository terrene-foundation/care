...
Run the full test suite; fix any import failures. With `pip install -e .` and `where = ["src"]`, imports should resolve as `pact.*` without change. This is validation, not bulk rewriting.
