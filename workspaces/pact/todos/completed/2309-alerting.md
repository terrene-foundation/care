...
- `src/pact/api/alerts.py` — `AlertDispatcher` class with `dispatch(event_type, payload)` method; HTTP POST logic with timeout and non-blocking failure handling
- `src/pact/config/env.py` — `CARE_ALERT_WEBHOOK_URL` env var declaration and documentation
