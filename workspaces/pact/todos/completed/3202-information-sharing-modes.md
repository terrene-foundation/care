...
Add a `sharing_policy: BridgeSharingPolicy | None = None` field to the `Bridge` model in `src/pact/workspace/bridge.py`.
...
- `src/pact/constraint/bridge_envelope.py` (add `SharingMode`, `FieldSharingRule`, `BridgeSharingPolicy`)
- `src/pact/workspace/bridge.py` (add `sharing_policy` field, integrate `check_field` into access control)
