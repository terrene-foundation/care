...
Replace the custom `pact.trust.messaging` implementation — which includes `MessageChannel`, `MessageRouter`, and `AgentMessage` cryptographic primitives — with `eatp.messaging.SecureChannel`. The PACT must not re-implement EATP cryptographic primitives; it must delegate to the SDK.
...
- `src/pact/trust/messaging.py` — migrate to `eatp.messaging.SecureChannel`; retain `MessageType` enum; add gap adapters with `# EATP-GAP` markers
