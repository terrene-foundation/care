...
- `src/pact/execution/approver_auth.py` — nonce write-on-consume, startup load
- `src/pact/persistence/store.py` — nonce persistence interface (add nonce read/write/delete operations if not already present)
