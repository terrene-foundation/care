...
`GenesisManager` implemented in `pact/trust/genesis.py` with full lifecycle management.

- `pact/trust/genesis.py` — `GenesisManager` with `create()`, `validate()`, `renew()`
...
Integrate EATP SDK genesis record creation into the PACT. The Genesis Record is the root of all trust.

## Tasks

- [x] `pact/trust/genesis.py` with `create_genesis_record()` → GenesisRecord
