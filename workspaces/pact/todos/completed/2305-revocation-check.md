...
- `src/pact/trust/revocation.py` — add a listener hook that, on revocation, identifies in-flight actions by agent ID and marks them
- `src/pact/constraint/middleware.py` — maintain a registry of in-flight action IDs (start time, agent ID) so revocation can query it; remove entries on completion
