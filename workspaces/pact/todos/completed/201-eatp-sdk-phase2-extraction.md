...
EATP SDK operations (ESTABLISH, DELEGATE, VERIFY, AUDIT) are available in the `eatp` package and fully integrated into the PACT via `EATPBridge`.

- `pact/trust/eatp_bridge.py` — bridge using `TrustOperations`, `InMemoryTrustStore`, `TrustKeyManager`
...
The PACT cannot function without the four EATP operations (ESTABLISH, DELEGATE, VERIFY, AUDIT). These exist inside Kailash Kaizen (Apache 2.0, Foundation-owned) but haven't been extracted to the standalone `eatp` package yet. This task extracts them.
