...
`DelegationManager` implemented in `pact/trust/delegation.py` with chain walking and monotonic tightening validation.

- `pact/trust/delegation.py` — `DelegationManager` with `create_delegation()`, `validate_tightening()`, `walk_chain()`
...
- [x] `pact/trust/delegation.py` with `create_delegation()` → DelegationRecord
