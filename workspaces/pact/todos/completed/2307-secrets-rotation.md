...
- `src/pact/trust/` — key versioning data model, retired keys store, grace period logic
- `src/pact/api/endpoints.py` — token rotation and key rotation endpoints
- `src/pact/cli/` — `rotate-token` and `rotate-signing-key` commands
