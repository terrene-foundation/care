...
Use existing `_is_time_window_tighter` and `_paths_covered_by` helper functions from `src/pact/constraint/envelope.py` where applicable.
...
- `src/pact/constraint/bridge_envelope.py` (add `validate_bridge_tightening()`)
- `src/pact/workspace/bridge.py` (call validator in `_activate()`)
