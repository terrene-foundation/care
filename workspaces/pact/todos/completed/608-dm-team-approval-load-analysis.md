...
- [ ] Design rapid-triage approval interface:
  - `pact list-held --team dm` must show in < 5 lines per action:
    1. Agent + action type
    2. Content preview (truncated to 200 chars)
    3. Why it is HELD
    4. Action ID for approve/reject
  - Batch approval: `pact approve-batch action-id-1 action-id-2 ...`
  - Approve-all (for routine items with low risk): `pact approve-routine --team dm`
