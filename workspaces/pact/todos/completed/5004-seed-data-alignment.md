...
Ensure `seed_demo.py` agent IDs, team IDs, and envelope IDs align with the canonical DM team config in `src/pact/verticals/dm_team.py`. Currently seed data may use IDs that don't match the DM team definition.

## Where

- `scripts/seed_demo.py` — align IDs with `dm_team.py`
- `src/pact/verticals/dm_team.py` — reference for canonical IDs
